...
# ======================== 认证网络 ========================
def build_verification_network_with_fusion(touch_base, git_base):
    """构建带跨模态融合的认证网络"""
    # 输入层
    ...
    diff = layers.Lambda(lambda x: tf.abs(x[0] - x[1]))([emb_a_features, emb_b_features])
    # 认证网络
...
if __name__ == "__main__":
    ...
    for account in range(1, 11):
        ...
        #保存整体的模型
        verification_model.save(f'E2_model/full_model_{label}_{account}.keras')
