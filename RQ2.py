...
PINs = load_PINs(csv_files_touch)
train_PINs, test_PINs = train_test_split(PINs, test_size=0.33, random_state=42)

class TripletDataGeneratorForTouch(Sequence):
    """用于三元损失的动态数据生成器"""
    def __init__(self, file_paths, users, PINs, batch_size=32):
    ...
    def __getitem__(self, idx):
        ...
        # # 打印形状信息
        # print(f"Batch {idx} data shape: {len(batch_triplets)}")
...
# class TripletAttentionDataGenerator(Sequence):
#     def __init__(self, touch_files, git_files, users, batch_size=32):
#         self.touch_files = touch_files
#         self.git_files = git_files
#         self.users = users
#         self.batch_size = batch_size
#         self.triplet_queue = []
#         for touch_file in self.touch_files:
...
#             else:
#                 continue  # 跳过不匹配的文件
#             git_file = next((f for f in self.git_files if f.endswith(git_file_suffix)), None)
#             if not git_file:
...
#                         continue
#                     triplet_pairs = self._generate_triplet_pairs(touch_user_samples, git_user_samples, user, touch_pin_group, git_pin_group)
#                     self.triplet_queue.extend(triplet_pairs)
#
#     def _generate_triplet_pairs(self, touch_user_samples, git_user_samples, user, touch_pin_group, git_pin_group):
#         """为单个用户生成正样本对"""
...
#         triplets = []
#         # # 为每个组合获取负样本
#         neg_users = [u for u in touch_pin_group['UUID'].unique() if u != user and u in self.users]
#
...
#
#             # 提取 touch 样本的特征
#             touch_features1 = touch_samples1[
#                 ['Time', 'X', 'Y', 'SizeMajor', 'SizeMinor', 'Orientation', 'Pressure', 'Size']].values
#             touch_features2 = touch_samples2[
#                 ['Time', 'X', 'Y', 'SizeMajor', 'SizeMinor', 'Orientation', 'Pressure', 'Size']].values
#
#             # 提取 git 样本的特征
...
#             neg_user_idx = np.random.choice(len(neg_users))
#             neg_user = neg_users[neg_user_idx]
#             neg_samples = touch_pin_group[touch_pin_group['UUID'] == neg_user]
#             grouped = neg_samples.groupby('Sample ID')
#             neg_samples = [group for _, group in grouped]
#             if not neg_samples:
#                 continue
#             neg_idx = np.random.choice(len(neg_samples))  # 先随机选择索引
#             neg_touch = neg_samples[neg_idx]  # 然后通过索引获取样本
#             neg_sample_id = neg_touch['Sample ID'].iloc[0]  # 取第一个值
#             neg_git = git_pin_group[git_pin_group['Sample ID'] == neg_sample_id]
#
#             neg_touch_feature = neg_touch[
#                 ['Time', 'X', 'Y', 'SizeMajor', 'SizeMinor', 'Orientation', 'Pressure', 'Size']].values
#             neg_git_feature = neg_git[['Time', 'X', 'Y', 'Z']].values
#
#             # 将提取的特征添加到正样本列表
#             triplets.append((touch_features1, git_features1, touch_features2, git_features2, neg_touch_feature, neg_git_feature))
#         return triplets
#
#     def __len__(self):
#         return int(len(self.triplet_queue) / self.batch_size)
...
#         end_idx = start_idx + self.batch_size
#         batch_triplets = self.triplet_queue[start_idx:end_idx]
#         anchors_touch, anchors_git, positives_touch, positives_git, negatives_touch, negatives_git = zip(*batch_triplets)
#         # 转换为 NumPy 数组
#         anchors_touch_array = np.array(anchors_touch)
#         anchors_git_array = np.array(anchors_git)
#         positives_touch_array = np.array(positives_touch)
#         positives_git_array = np.array(positives_git)
#         negatives_touch_array = np.array(negatives_touch)
#         negatives_git_array = np.array(negatives_git)
#         return (anchors_touch_array, anchors_git_array, positives_touch_array, positives_git_array, negatives_touch_array, negatives_git_array), np.zeros(len(batch_triplets))

class VerificationDataGenerator(Sequence):
    """用于认证网络的动态数据生成器"""
    def __init__(self, csv_touch_files, csv_git_files, users, PINs, batch_size=32):
...
#
#     # 只选择满足三元组条件的样本
#     mask = tf.math.logical_and(pos_dist < neg_dist,
#                                neg_dist < pos_dist + alpha)
#     loss = tf.maximum(0.0, pos_dist - neg_dist + alpha)
...
if __name__ == "__main__":
    ...
    train_gen_touch = TripletDataGeneratorForTouch(csv_files_touch, train_users, train_PINs,
    ...
    touch_triplet_model.save(f'E4_model/touch_triplet_model_{label}.keras')
    # 构建步态双胞胎网络
    ...
    train_gen_git = TripletDataGeneratorForGit(csv_files_git, train_users, train_PINs,
                                               batch_size=siamese_param_grid['batch_size'])
    lr_scheduler = LearningRateScheduler(reduce_after_epoch=reduce_after_epoch)
    git_triplet_model.fit(train_gen_git, epochs=total_epochs, callbacks=[lr_scheduler])
    git_triplet_model.save(f'E4_model/git_triplet_model_{label}.keras')
    ...
    train_gen = VerificationDataGenerator(csv_files_touch, csv_files_git, train_users, PINs,
    ...
    # 保存整体的模型
    verification_model.save(f'E4_model/full_model_{label}.keras')

    # 5. 测试每个CSV文件并保存结果
    test_results = {}
    export = {"Type": [], "User": [], "Posture": [], "PIN": [], "Fpr": [], "Tpr": [], "EER": [], "EER_Threshold": [],
              "Thresholds": [], "Labels": [], "Predictions": []}
    output_dir = 'E4_results'  # 测试结果保存目录
    ...
    for touch_file in csv_files_touch:
        ...
        for PIN in test_PINs:
            ...
            for user in test_users:
                ...
                export["Labels"].append(labels)
                export["Predictions"].append(predictions)
    results_file_ftt = os.path.join(output_dir, f'test_results_{label}.csv')
