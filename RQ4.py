...
if __name__ == "__main__":
    ...
    for account in range(1, 11):
        ...
        # 1. 加载完整模型
        full_model = load_model(f'E1_model/full_model_with_feature_fusion_{account}.keras', compile=True)
        # 2. 获取特征融合网络
        feature_fusion_model = full_model.get_layer("feature_fusion_network")
        # 3. 创建特征提取模型
        touch_input = layers.Input(shape=(input_data_length_touch, input_feature_length_touch))
        git_input = layers.Input(shape=(input_data_length_git, input_feature_length_git))
        # 获取基础模型
        touch_base = full_model.get_layer("touch_base_network")
        git_base = full_model.get_layer("git_base_network")
        # 提取特征并融合
        emb_touch = touch_base(touch_input)
        emb_git = git_base(git_input)
        concat_features = layers.Concatenate()([emb_touch, emb_git])
        fused_features = feature_fusion_model(concat_features)
        # 构建特征提取模型
        feature_extraction_model = models.Model(
            inputs=[touch_input, git_input],
            outputs=fused_features,
            name=f"feature_extractor_account_{account}"
        )
        # 5. 处理每个CSV文件
        export = {"User": [], "Posture": [], "PIN": [], "Sample_ID": [], "feature_vector": []}
        output_dir = 'E7_results'  # 测试结果保存目录
        for touch_file in csv_files_touch:
            # 确定姿势类型
            if "sit" in touch_file:
                posture = "sit"
                git_file = 'datas/masked_normalized_filtered_sensor_data_sit.csv'
            elif "walk" in touch_file:
                posture = "walk"
                git_file = 'datas/masked_normalized_filtered_sensor_data_walk.csv'
            else:
                continue
            print(f"Processing {posture} data...")
            # 读取数据
            touch_df = pd.read_csv(touch_file)
            git_df = pd.read_csv(git_file)
            # 获取共同PIN
            common_pins = set(touch_df['PIN']).intersection(set(git_df['PIN']))
            for pin in common_pins:
                print(f"Processing PIN: {pin}")
                # 筛选当前PIN的数据
                pin_touch = touch_df[touch_df['PIN'] == pin]
                pin_git = git_df[git_df['PIN'] == pin]
                # 获取共同样本ID
                common_sample_ids = set(pin_touch['Sample ID']).intersection(set(pin_git['Sample ID']))
                for sample_id in common_sample_ids:
                    # 提取当前样本的touch数据
                    sample_touch = pin_touch[pin_touch['Sample ID'] == sample_id]
                    # 确保数据长度正确
                    if len(sample_touch) != input_data_length_touch:
                        continue
                    # 提取特征值
                    touch_features = sample_touch[
                        ['Time', 'X', 'Y', 'SizeMajor', 'SizeMinor', 'Orientation', 'Pressure', 'Size']
                    ].values
                    # 提取当前样本的git数据
                    sample_git = pin_git[pin_git['Sample ID'] == sample_id]
                    # 确保数据长度正确
                    if len(sample_git) != input_data_length_git:
                        continue
                    # 提取特征值
                    git_features = sample_git[['Time', 'X', 'Y', 'Z']].values
                    # 获取UUID（取第一个）
                    uuid = sample_touch['UUID'].iloc[0] if 'UUID' in sample_touch.columns else "unknown"
                    # 提取特征向量
                    touch_input = np.expand_dims(touch_features, axis=0)
                    git_input = np.expand_dims(git_features, axis=0)
                    feature_vector = feature_extraction_model.predict([touch_input, git_input], verbose=0)[0]
                    # 压缩特征向量为字符串表示
                    # 使用科学计数法保留6位有效数字
                    export["User"].append(uuid)
                    export["Posture"].append(posture)
                    export["PIN"].append(pin)
                    export["Sample_ID"].append(sample_id)
                    export["feature_vector"].append(feature_vector)
        results_file_ftt = os.path.join(output_dir, f'feature_vectors_{account}.csv')
